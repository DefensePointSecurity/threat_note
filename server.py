...
def campaignsummary(uid):
    try:
        ...
        if str(http['inputtype']) == "IPv4" or str(http['inputtype']) == "IPv6" or str(http['inputtype']) == "Domain" or str(http['inputtype']) == "Network":
...
def updateobject():
    try:
        ...
        if newdict['inputtype'] == "Threat Actor":
            return render_template('threatactorobject.html', records=http, jsonvt=jsonvt, whoisdata=whoisdata,settingsvars=settingsvars)
        else:
            return render_template('networkobject.html', records=http, jsonvt=jsonvt, whoisdata=whoisdata,settingsvars=settingsvars)
    except Exception as e:
        return render_template('error.html', error=e)
...
def objectsummary(uid):
    try:
        ...
        return render_template('networkobject.html', records=http, jsonvt=jsonvt, whoisdata=whoisdata,settingsvars=settingsvars, address=address)
    except Exception as e:
        return render_template('error.html', error=e)
...
@app.context_processor
def networkcount():
    return dict(networkcount=mongo.db.network.find({"$or": [{"inputtype": "IPv4"}, {"inputtype": "IPv6"}, {"inputtype":"Network"}, {"inputtype": "Domain"}]}).count())
